"""CLI Password Manager — Day 38 Python | Author: devashmit"""
...
def derive_key(pw,salt):kdf=PBKDF2HMAC(algorithm=hashes.SHA256(),length=32,salt=salt,iterations=480_000);return base64.urlsafe_b64encode(kdf.derive(pw.encode()))
def load():return json.loads(VAULT.read_text()) if VAULT.exists() else{"salt":None,"entries":{}}
def save(v):VAULT.write_text(json.dumps(v,indent=2))
def fernet(v,pw):return Fernet(derive_key(pw,bytes.fromhex(v["salt"])))
...
def cmd_add(_):
    ...
    v["entries"][site]={"username":user,"password":f.encrypt(passwd.encode()).decode()};save(v);print(f"  Saved '{site}'.")
...
def cmd_delete(_):
    ...
    if site not in v["entries"]:print("Not found.");return
    pw=getpass.getpass("Master passphrase: ")
    try:fernet(v,pw).decrypt(v["entries"][site]["password"].encode());del v["entries"][site];save(v);print(f"  Deleted '{site}'.")
    except InvalidToken:print("Wrong passphrase.")
