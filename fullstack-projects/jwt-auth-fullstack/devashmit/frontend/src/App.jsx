...
export default function App(){
  ...
  if(user)return(
    <div className="card"><div className="dash">
      <span style={{fontSize:"2.5rem"}}>🎉</span>
      <h1>Dashboard</h1><p>Logged in as</p>
      <p className="dash__user">@{user}</p>
      <p className="dash__note">Token in httpOnly cookie — XSS safe</p>
  ...
  return(
    <div className="card">
      ...
      <form onSubmit={submit}>
        <div className="form-group"><label>Username</label><input value={u} onChange={e=>setU(e.target.value)} placeholder="username" required autoFocus/></div>
        <div className="form-group"><label>Password</label><input type="password" value={p} onChange={e=>setP(e.target.value)} placeholder="••••••" required/></div>
        {err&&<p className="err">⚠️ {err}</p>}
        <button className="btn btn--primary" disabled={busy}>{busy?"…":view==="login"?"Sign In":"Create Account"}</button>
      </form>
      <button className="link-btn" onClick={()=>{setView(v=>v==="login"?"register":"login");setErr("");}}>
