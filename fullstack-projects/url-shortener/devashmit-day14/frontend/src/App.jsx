...
export default function App(){
  ...
  const fetchAnalytics=useCallback(async()=>{
    try{const r=await fetch(`${API}/analytics`);if(r.ok)setAnalytics(await r.json());}catch{}
  },[]);
  ...
  const copy=()=>{
    if(!result)return;
    navigator.clipboard.writeText(result.short_url).then(()=>{setCopied(true);setTimeout(()=>setCopied(false),2000);});
  };
  ...
  return(
    <div className="app">
      ...
      <form className="form" onSubmit={submit}>
        <input type="url" placeholder="https://example.com/long/url" value={url} onChange={e=>setUrl(e.target.value)} aria-label="URL to shorten"/>
        <button className="btn" disabled={loading||!url.trim()}>{loading?"…":"Shorten"}</button>
      </form>
      {error&&<p className="error" role="alert">⚠️ {error}</p>}
      {result&&(
        <div className="result">
          <a href={result.short_url} target="_blank" rel="noopener noreferrer">{result.short_url}</a>
          <button className={`copy-btn${copied?" copied":""}`} onClick={copy}>{copied?"✓ Copied":"Copy"}</button>
        </div>
      )}
      <section className="analytics" aria-label="Analytics">
        <h2>Analytics</h2>
