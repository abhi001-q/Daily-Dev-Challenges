...
export default function NoteFormPage(){
  ...
  const submit=async e=>{
    ...
    try{const r=await fetch(isEdit?`${API}/notes/${id}`:`${API}/notes`,{method:isEdit?"PUT":"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({title:title.trim(),content})});
      if(!r.ok)throw new Error("Save failed");const n=await r.json();navigate(`/notes/${n.id}`);
  ...
  return(<div className="layout">
    ...
    <form onSubmit={submit}>
      ...
      <div className="form-actions">
        <button className="btn btn--primary" disabled={saving}>{saving?"Saving…":isEdit?"Save Changes":"Create Note"}</button>
