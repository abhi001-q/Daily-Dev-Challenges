...
export default function NoteFormPage(){
  ...
  useEffect(()=>{if(!isEdit)return;fetch(`${API}/notes/${id}`).then(r=>r.json()).then(n=>{setTitle(n.title);setContent(n.content);}).catch(()=>navigate("/notes"));},[id,isEdit,navigate]);
  const submit=async e=>{
    ...
    try{const r=await fetch(isEdit?`${API}/notes/${id}`:`${API}/notes`,{method:isEdit?"PUT":"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({title:title.trim(),content})});
      if(!r.ok)throw new Error("Failed to save");const n=await r.json();navigate(`/notes/${n.id}`);
    }catch(e){setError(e.message);}finally{setSaving(false);}
  };
  return(<div className="layout">
    <button className="back-link" onClick={()=>navigate(isEdit?`/notes/${id}`:"/notes")}>← Back</button>
