"""Real-time Chat with WebSockets — Day 14 FastAPI | Author: devashmit"""
...
<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:system-ui,sans-serif;background:#0f1117;color:#f0f2ff;min-height:100vh;display:flex;flex-direction:column;align-items:center;padding:2rem 1rem}h1{font-size:1.6rem;font-weight:800;margin-bottom:1.5rem}.setup{display:flex;gap:.5rem;margin-bottom:1rem;width:100%;max-width:600px}input{flex:1;background:#1a1d2e;border:1px solid #2e3250;border-radius:8px;padding:.65rem 1rem;color:#f0f2ff;font-size:.9rem;outline:none}input:focus{border-color:#6366f1}button{background:#4f46e5;color:#fff;border:none;border-radius:8px;padding:.65rem 1.2rem;font-weight:700;cursor:pointer}button:hover{background:#4338ca}#messages{width:100%;max-width:600px;height:350px;overflow-y:auto;background:#1a1d2e;border:1px solid #2e3250;border-radius:12px;padding:1rem;margin-bottom:1rem;display:flex;flex-direction:column;gap:.5rem}.msg{font-size:.88rem;line-height:1.4}.system{color:#8b92b8;font-style:italic}.mine{color:#818cf8;font-weight:600}.send-row{display:flex;gap:.5rem;width:100%;max-width:600px}#status{font-size:.8rem;color:#8b92b8;margin-bottom:.75rem}</style></head>
<body>
...
<script>
let ws=null,myName="";
function addMsg(text,cls=""){const d=document.getElementById("messages");const p=document.createElement("p");p.className="msg "+cls;p.textContent=text;d.appendChild(p);d.scrollTop=d.scrollHeight;}
function connect(){if(ws)ws.close();const room=document.getElementById("room").value.trim()||"general";myName=document.getElementById("username").value.trim()||"User";ws=new WebSocket(`ws://localhost:8000/ws/${room}/${encodeURIComponent(myName)}`);ws.onopen=()=>{document.getElementById("status").textContent=`Connected to #${room} as ${myName}`;};ws.onmessage=e=>{const isSystem=e.data.startsWith("🟢")||e.data.startsWith("🔴");const isMine=e.data.startsWith(`${myName}:`);addMsg(e.data,isSystem?"system":isMine?"mine":"");};ws.onclose=()=>{document.getElementById("status").textContent="Disconnected";};}
function send(){const inp=document.getElementById("msgInput");const msg=inp.value.trim();if(msg&&ws&&ws.readyState===1){ws.send(msg);inp.value="";}}
</script></body></html>""")

@app.get("/rooms")
async def list_rooms():
    return [{"room":r,"users":len(c)} for r,c in manager.rooms.items()]
