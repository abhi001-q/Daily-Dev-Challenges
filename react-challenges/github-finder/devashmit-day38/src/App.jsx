    ...
    {!loading&&user&&(
      <div className="profile">
        <div className="profile__header">
          ...
          <div><p className="profile__name">{user.name||user.login}</p>
        ...
        {top5&&top5.length>0&&(<div className="repos"><h3>Top Repositories</h3>
          <ul>{top5.map(r=><li key={r.id} className="repo"><a href={r.html_url} target="_blank" rel="noopener noreferrer">📁 {r.name}</a>{r.description&&<p className="repo__desc">{r.description}</p>}<div className="repo__meta"><span>⭐ {r.stargazers_count}</span>{r.language&&<span>🔵 {r.language}</span>}</div></li>)}</ul>
        </div>)}
      </div>
    )}
