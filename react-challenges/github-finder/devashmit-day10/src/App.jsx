...
export default function App() {
  ...
  const { data: repos, loading: rl } = useFetch(user ? `${GH}/users/${query}/repos?per_page=100&sort=pushed` : null);
  const loading = ul || rl;
  ...
  return (
    <div className="app">
      <div className="app__header">
        ...
        <p>Search any GitHub username</p>
      </div>
      <form className="search-bar" onSubmit={e=>{e.preventDefault();if(username.trim())setQuery(username.trim());}}>
        <input value={username} onChange={e=>setUsername(e.target.value)} placeholder="Search username…" aria-label="GitHub username" autoFocus/>
